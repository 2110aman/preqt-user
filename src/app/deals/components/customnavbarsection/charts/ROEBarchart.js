...
const ROEBarchart = ({ isPrivate, data: apiData }) => {
    // Transform API data to chart format with error handling
    const transformData = (data) => {
        try {
            if (!data || !Array.isArray(data) || data.length === 0) {
                console.warn('ROEBarchart: No valid data provided, using fallback');
                return [];
            }
    ...
    return (
        <ResponsiveContainer width="100%" height={320}>
            <BarChart data={chartData} barSize={60} margin={{ top: 25, right: 10, left: 15, bottom: 20 }}>
                <defs>
                    <linearGradient id="roeLatestBarGradient" x1="0" y1="0" x2="0" y2="1">
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <p style={{ margin: 0, color: "#B57D23", fontSize: 13, fontWeight: "500" }}>
                                        ROE: {Number(payload[0].value).toFixed(1)}%
                                    </p>
                                </div>
                            );
                        }
                ...
                >
                    {chartData.map((entry, index) => {
                        ...
                        // For ROE (%), both negative and positive prior years are solid pale gold (#F5E3B2).
                        // The latest year is the gold gradient.
                        const fillVal = isLatest ? "url(#roeLatestBarGradient)" : "#F5E3B2";
                        return (
                            <Cell
                                key={`cell-${index}`}
                                fill={fillVal}
